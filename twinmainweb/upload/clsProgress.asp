...
' ------------------------------------------------------------------------------
	Public Sub DeleteOldFiles()
		...
		Dim DateCheck		' Date file must have been modified after

		' Setup date to be 1 hour ago.		
		DateCheck = DateAdd("n", -1, Now())
		
		' Get folder with temp files
		Set Folder = FSO.GetSpecialFolder(TemporaryFolder)

		' Loop through each file
		For Each File In Folder.Files

			' If file is used with AspUpload ...
			If InStr(1, File.Name, "AspUpload_") = 1 Then

				' If file has not been modified for a while
				If File.DateLastModified < DateCheck Then
